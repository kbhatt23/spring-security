...
public class DigestAuthenticationConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication().withUser("kbhatt23").password(passwordEncoder().encode("jaishreeram"))
				.roles("USER").and().withUser("raghava").password(passwordEncoder().encode("adminsecret")).roles("ADMIN");
	}
