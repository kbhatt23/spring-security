...
public class BasicAuthSessionCookieFilter extends OncePerRequestFilter {

	private final BasicAuthUserRepository basicAuthUserRepository;


	public BasicAuthSessionCookieFilter(BasicAuthUserRepository basicAuthUserRepository){
	...
	private boolean isValidUsernameAndPassword(String authHeader, HttpServletRequest request) {
		...
		try {
			...
			String encryptedUserName = EncryptDecryptUtil.encryptAes(userName.toLowerCase(), BasicAuthUserController.SECRET);
			...
			if (findByUsername.isPresent()) {
				...
				if( HashUtil.isSha256Match(password, basicAuthUser.getSalt(), basicAuthUser.getPasswordHash())) {
					request.setAttribute(SessionCookieConstants.REQUEST_ATTRIBUTE_USERNAME, encryptedUserName);
					return true;
				}else
				{
					return false;
