...
public class RedisSessionTokenLoginFilter extends OncePerRequestFilter {

	private final BasicAuthUserRepository basicAuthUserRepository;


	public RedisSessionTokenLoginFilter(BasicAuthUserRepository basicAuthUserRepository){
		...
		try {
			...
			if (findByUsername.isPresent()) {
				...
				if( HashUtil.isSha256Match(password, basicAuthUser.getSalt(), basicAuthUser.getPasswordHash())) {
					request.setAttribute(RedisAuthConstants.REQUEST_ATTRIBUTE_USERNAME, encryptedUserName);
					return true;
				}else
				{
					return false;
